...
namespace Cqrs.Portable.Tests.AtomicStorage
{
    ...
    public class NonserializingMemoryDocumentStoreTest : DocumentStoreTest
    {
        private ConcurrentDictionary<string, ConcurrentDictionary<string, object>> _storeDictionary;
        ...
        public void reset_all_bucket()
        {
            ...
            Store.WriteContents(bucket1, records);
            Store.WriteContents(bucket2, records);
            ((NonserializingMemoryDocumentStore)Store).ResetAll();
    ...
    public abstract class DocumentStoreTest
    {
        ...
        public void write_bucket()
        {
            ...
            for (int i = 0; i < records.Count; i++)
            {
	            ...
	            var actualRecord = actualRecords.First( x => x.Key == records[ i ].Key );
	            Assert.AreEqual(Encoding.UTF8.GetString(records[i].Read()), Encoding.UTF8.GetString(actualRecord.Read()));
            }
        }

	    [ Test ]
	    public void read_write_doc()
	    {
		    for( var i = 0; i < 25; i++ )
		    {
			    this.Store.GetWriter< string, string >().AddOrUpdate( "key" + i, () => "val" + i, s => "val" + i, AddOrUpdateHint.ProbablyDoesNotExist );
		    }

		    for( var i = 0; i < 25; i++ )
		    {
			    var actualValue = this.Store.GetReader< string, string >().Load( "key" + i );
			    Assert.AreEqual( "val" + i, actualValue );
		    }
	    }
